			...
			<tr>
				...
				<td>
					<div class="no-margin text-bold">
						<a class="kq_search" href="{{ action('PRSubscriberController@edit', ['p_r_list_uid' => $list->uid ,'uid' => $item->uid]) }}">
							{{ $item->email }}
						</a>
						<br />
				...
				<td class="text-right text-nowrap">
					@if (\Gate::allows('update', $item))
						<a href="{{ action('PRSubscriberController@edit', ['p_r_list_uid' => $list->uid, "uids" => $item->uid]) }}" type="button" class="btn bg-grey btn-icon">
					...
					<div class="btn-group">
						...
						<ul class="dropdown-menu dropdown-menu-right">
							@if (\Gate::allows('subscribe', $item))
								<li><a class="ajax_link" href="{{ action('PRSubscriberController@subscribe', ['p_r_list_uid' => $list->uid, "uids" => $item->uid]) }}"><i class="icon-enter"></i> {{ trans('messages.subscribe') }}</a></li>
							@endif
							@if (\Gate::allows('unsubscribe', $item))
								<li><a class="ajax_link" href="{{ action('PRSubscriberController@unsubscribe', ['p_r_list_uid' => $list->uid, "uids" => $item->uid]) }}"><i class="icon-exit"></i> {{ trans('messages.unsubscribe') }}</a></li>
							@endif

							<li>
								<a href="#copy" class="copy_move_subscriber"
									data-url="{{ action('PRSubscriberController@copyMoveForm', [
										'uids' => $item->uid,
										'p_r_list_uid' => $list->uid,
										'action' => 'copy',
									]) }}">
							...
							@if (\Gate::allows('delete', $item))
								<li><a delete-confirm="{{ trans('messages.delete_subscribers_confirm') }}" href="{{ action('PRSubscriberController@delete', ['p_r_list_uid' => $list->uid, "uids" => $item->uid]) }}"><i class="icon-trash"></i> {{ trans("messages.delete") }}</a></li>
							@endif
						</ul>
					</div>
				</td>
