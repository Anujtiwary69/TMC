...
@section('content')
                ...
                <form action="{{ action('PRFieldController@store', $list->uid) }}" class="listing-form"                    
					sort-urla="{{ action('PRFieldController@sort', $list->uid) }}"
					per-page="1"
					method="POST"
                >
					{{ csrf_field() }}
                        ...
                        >
                            ...
                            @foreach ($fields as $key => $item)
                                <tr class="draggable" rel="{{ $item->uid }}">
                                    ...
                                    <td class="text-nowrap">
										@include('helpers.form_control', ['type' => Acelle\Model\PRField::getControlNameByType($item->type), 'name' => 'fields[' . $item->uid . '][default_value]', 'label' => '', 'value' => $item->default_value, 'help_class' => 'field'])										
									</td>
									<td>
										@if (is_object(Acelle\Model\PRField::findByUid($item->uid)))
											<a no-ajax="true" href="{{ action('PRFieldController@delete', ['list_uid' => $list->uid, 'uid' => $item->uid]) }}" delete-confirm="{!! trans('messages.delete_field_alert') !!}" class="btn bg-danger-400 remove-field-button">
					...
					<div>                                    
						<span sample-url="{{ action("PRFieldController@sample", ['list_uid' => $list->uid, "type" => "text"]) }}" class="btn btn-default btn-xs add-custom-field-button mr-10" type_name="text">
							<i class="icon-font-size2"></i> {{ trans('messages.text_field') }}
						</span>
						<span sample-url="{{ action("PRFieldController@sample", ['list_uid' => $list->uid, "type" => "number"]) }}" class="btn btn-default btn-xs add-custom-field-button mr-10" type_name="number">
							<i class="icon-sort-numeric-asc"></i> {{ trans('messages.number_field') }}
						</span>
						<span sample-url="{{ action("PRFieldController@sample", ['list_uid' => $list->uid, "type" => "dropdown"]) }}" class="btn btn-default btn-xs add-custom-field-button mr-10" type_name="dropdown">
							<i class="icon-menu2"></i> {{ trans('messages.dropdown_field') }}
						</span>
						<span sample-url="{{ action("PRFieldController@sample", ['list_uid' => $list->uid, "type" => "multiselect"]) }}" class="btn btn-default btn-xs add-custom-field-button mr-10" type_name="multiselect"><i class="icon-menu3"></i> {{ trans('messages.multiselect_field') }}</span>
						<span sample-url="{{ action("PRFieldController@sample", ['list_uid' => $list->uid, "type" => "checkbox"]) }}" class="btn btn-default btn-xs add-custom-field-button mr-10" type_name="checkbox"><i class="icon-checkbox-partial2"></i> {{ trans('messages.checkbox_field') }}</span>
						<span sample-url="{{ action("PRFieldController@sample", ['list_uid' => $list->uid, "type" => "radio"]) }}" class="btn btn-default btn-xs add-custom-field-button mr-10" type_name="radio"><i class="icon-circles"></i> {{ trans('messages.radio_field') }}</span>
						<span sample-url="{{ action("PRFieldController@sample", ['list_uid' => $list->uid, "type" => "date"]) }}" class="btn btn-default btn-xs add-custom-field-button mr-10" type_name="date"><i class="icon-calendar52"></i> {{ trans('messages.date_field') }}</span>
						<span sample-url="{{ action("PRFieldController@sample", ['list_uid' => $list->uid, "type" => "datetime"]) }}" class="btn btn-default btn-xs add-custom-field-button mr-10" type_name="datetime"><i class="icon-alarm"></i> {{ trans('messages.datetime_field') }}</span>
						<span sample-url="{{ action("PRFieldController@sample", ['list_uid' => $list->uid, "type" => "textarea"]) }}" class="btn btn-default btn-xs add-custom-field-button" type_name="textarea"><i class="icon-menu6"></i> {{ trans('messages.textarea_field') }}</span>
					</div>					
