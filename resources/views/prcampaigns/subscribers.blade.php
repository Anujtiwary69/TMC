...
@section('page_header')
			...
			@include("prcampaigns._header")

@endsection

@section('content')

      @include("prcampaigns._menu")

			<h2 class="text-semibold text-teal-800">{{ trans('messages.tracking_log') }}</h2>

			<form class="listing-form"
					data-url="{{ action('PRCampaignController@subscribersListing', $campaign->uid) }}"
					per-page="{{ Acelle\Model\PRSubscriber::$itemsPerPage }}"
				>
								...
								<div class="filter-box">
									<div class="btn-group list_actions hide mr-10">
										...
										<ul class="dropdown-menu">
											<li>
												<a link-confirm="{{ trans('messages.subscribe_subscribers_confirm') }}" href="{{ action('PRSubscriberController@subscribe', $list->uid) }}">
													<i class="icon-enter"></i> {{ trans('messages.subscribe') }}
												</a>
											</li>
											<li>
												<a link-confirm="{{ trans('messages.unsubscribe_subscribers_confirm') }}" href="{{ action('PRSubscriberController@unsubscribe', $list->uid) }}">
												...
												<ul>
													<li>
														...
														<ul>
															@forelse ($list->otherLists() as $l)
																<li>
																	<a link-confirm="{{ trans('messages.subscribers_copy_confirm.replace') }}" href="{{ action('PRSubscriberController@copy', ['list_uid' => $l->uid, 'type' => 'update']) }}">
													...
													<li>
														...
														<ul>
															@forelse ($list->otherLists() as $l)
																<li>
																	<a link-confirm="{{ trans('messages.subscribers_copy_confirm.keep') }}" href="{{ action('PRSubscriberController@copy', ['list_uid' => $l->uid, 'type' => 'keep']) }}">
																		<i class="icon-address-book2"></i> {{ $l->name }}
																	</a>
																</li>
															@empty
												...
												<ul>
													<li>
														...
														<ul>
															@forelse ($list->otherLists() as $l)
																<li>
																	<a link-confirm="{{ trans('messages.subscribers_move_confirm.replace') }}" href="{{ action('PRSubscriberController@move', ['list_uid' => $l->uid, 'type' => 'update']) }}">
																		<i class="icon-address-book2"></i> {{ $l->name }}
																	</a>
																</li>
															@empty
													...
													<li>
														...
														<ul>
															@forelse ($list->otherLists() as $l)
																<li>
																	<a link-confirm="{{ trans('messages.subscribers_move_confirm.keep') }}" href="{{ action('PRSubscriberController@move', ['list_uid' => $l->uid, 'type' => 'keep']) }}">
																		<i class="icon-address-book2"></i> {{ $l->name }}
																	</a>
																</li>
															@empty
											...
											<li>
												<a delete-confirm="{{ trans('messages.delete_subscribers_confirm') }}" href="{{ action('PRSubscriberController@delete', $list->uid) }}">
