...
@section('page_header')
			...
			<div class="page-title">
				...
				@include('prcampaigns._steps', ['current' => 1])
			</div>

@endsection

@section('content')
                <form action="{{ action('PRCampaignController@recipients', $campaign->uid) }}" method="POST" class="form-validate-jqueryz">
					{{ csrf_field() }}
					...
					<div class="addable-multiple-form">
						<div class="addable-multiple-container">
							...
							@foreach ($campaign->getListsSegmentsGroups() as $index =>  $lists_segment_group)
								@include('prcampaigns._list_segment_form', [
						...
						<br />
						<a
							sample-url="{{ action('PRCampaignController@listSegmentForm', $campaign->uid) }}"
