			...
			<tr>
				...
				<td>
					<h5 class="no-margin text-bold">
						<a class="kq_search" href="{{ action('PRCampaignController@show', $item->uid) }}">
							{{ $item->name }}
						</a>
					</h5>
				...
				<td class="text-right">
					<span class="text-muted2 list-status pull-left" title='{{ $item->status == Acelle\Model\PRCampaign::STATUS_ERROR ? $item->last_error : '' }}' data-popup='tooltip'>
						<span class="label label-flat bg-{{ $item->status }}">{{ trans('messages.campaign_status_' . $item->status) }}</span>
					</span>
					<!-- @if (\Gate::allows('update', $item)) -->
						<a href="{{ action('PRCampaignController@edit', $item->uid) }}" type="button" class="btn bg-grey btn-icon"> <i class="icon-pencil"></i> {{ trans('messages.edit') }}</a>
					<!-- @endif -->
					<!-- @if (\Gate::allows('overview', $item)) -->
						<a href="{{ action('PRCampaignController@overview', $item->uid) }}" data-popup="tooltip" title="{{ trans('messages.overview') }}" type="button" class="btn bg-teal-600 btn-icon"><i class="icon-stats-growth"></i> {{ trans('messages.overview') }}</a>
					<!-- @endif -->
					<!-- @if (\Gate::allows('delete', $item) || \Gate::allows('pause', $item) || \Gate::allows('restart', $item)) -->
						<div class="btn-group">
							...
							<!-- 	@if (\Gate::allows('pause', $item)) -->
									<li><a link-confirm="{{ trans('messages.pause_campaigns_confirm') }}" href="{{ action('PRCampaignController@pause', ["uids" => $item->uid]) }}"><i class="icon-pause"></i> {{ trans("messages.pause") }}</a></li>
								<!-- @endif -->
								<!-- @if (\Gate::allows('restart', $item)) -->
									<li><a link-confirm="{{ trans('messages.restart_campaigns_confirm') }}" href="{{ action('PRCampaignController@restart', ["uids" => $item->uid]) }}"><i class="icon-history"></i> {{ trans("messages.restart") }}</a></li>
								<!-- @endif
							...
							<!-- 	@endif
								@if (\Gate::allows('delete', $item)) -->
									<li><a delete-confirm="{{ trans('messages.delete_campaigns_confirm') }}" href="{{ action('PRCampaignController@delete', ["uids" => $item->uid]) }}"><i class="icon-trash"></i> {{ trans("messages.delete") }}</a></li>
								<!-- @endif -->
							</ul>
						</div>
					<!-- @endif -->
