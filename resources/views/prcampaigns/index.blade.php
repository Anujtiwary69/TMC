...
@section('content')
    <form class="listing-form"
        sort-url="{{ action('PRCampaignController@sort') }}"
        data-url="{{ action('PRCampaignController@listing') }}"
        per-page="{{ Acelle\Model\PRList::$itemsPerPage }}"
    >
        <div class="row top-list-controls">
                    ...
                    <div class="filter-box">
                        <div class="btn-group list_actions hide">
                            ...
                            <ul class="dropdown-menu">
                                <li><a link-confirm="{{ trans('messages.restart_campaigns_confirm') }}" href="{{ action('PRCampaignController@restart') }}"><i class="icon-history"></i> {{ trans("messages.restart") }}</a></li>
                                <li><a link-confirm="{{ trans('messages.pause_campaigns_confirm') }}" href="{{ action('PRCampaignController@pause') }}"><i class="icon-pause"></i> {{ trans("messages.pause") }}</a></li>
                                <li><a delete-confirm="{{ trans('messages.delete_campaigns_confirm') }}" href="{{ action('PRCampaignController@delete') }}"><i class="icon-trash"></i> {{ trans('messages.delete') }}</a></li>
                            </ul>
                        </div>
            ...
            <div class="col-md-2 text-right">
                <a href="{{ action('PRCampaignController@selectType') }}" type="button" class="btn bg-info-800">
