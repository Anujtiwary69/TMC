...
class PRSegment extends Model
{
    ...
    {
        return $this->hasMany('Acelle\Model\PRSegmentCondition');
    }
    ...
    public static function filter($request)
    {
        ...
        $query = self::select('p_r_segments.*')->where("p_r_segments.p_r_list_id", "=", $list->id);
    ...
    {
        $query = \Acelle\Model\PRSubscriber::select('subscribers.*', 'email_verifications.result AS verify_result');
        $query = PRSubscriber::filter($query, $request);
        ...
        // WHERE...
        $query = $query->where('subscribers.p_r_list_id', $this->mail_list_id);
    ...
    {
        return $this->subscribers()->count('prsubscribers.id');
    }
}
