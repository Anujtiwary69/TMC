...
class PRField extends Model
{
     ...
     */
    protected $fillable = [
        'p_r_list_id', 'type', 'label', 'tag', 'default_value', 'visible', 'required', 'custom_order',
    ];
    ...
    {
        return $this->hasMany('Acelle\Model\PRFieldOption');
    }
    ...
    public static function boot()
    {
        ...
        static::creating(function ($item) {
            // Create new uid
            $uid = uniqid();
            while (PRField::where('uid', '=', $uid)->count() > 0) {
                $uid = uniqid();
            }
            $item->uid = $uid;
        });
