...
class PRList extends Model
{
    ...
    {
        return $this->hasMany('Acelle\Model\PRField');
    }
    ...
    {
        return $this->hasMany('Acelle\Model\PRSegment');
    }

    public function pages()
    {
        return $this->hasMany('Acelle\Model\PRPage');
    }
    ...
    {
        return $this->hasMany('Acelle\Model\PRSubscriber');
    }
    ...
    public function campaigns()
    {
        ...
        return $this->belongsToMany('Acelle\Model\PRCampaign', 'p_r_campaigns_lists_segments', 'p_r_list_id', 'p_r_campaign_id');
    }
    ...
    public static function boot()
    {
        ...
        static::creating(function ($item) {
            // Create new uid
            $uid = uniqid();
            while (PRList::where('uid', '=', $uid)->count() > 0) {
                $uid = uniqid();
            }
            $item->uid = $uid;

            // Update custom order
            PRList::getAll()->increment('custom_order', 1);
            $item->custom_order = 0;
        });
    ...
    public function createDefaultFieds()
    {
        ...
        $this->fields()->create([
                            ...
                            'tag' => \Acelle\Model\PRField::formatTag(trans('messages.first_name_tag')),
                            'required' => false,
                            'visible' => true,
                        ]);
        ...
        $this->fields()->create([
                            ...
                            'tag' => \Acelle\Model\PRField::formatTag(trans('messages.last_name_tag')),
                            'required' => false,
                            'visible' => true,
                        ]);
    }
