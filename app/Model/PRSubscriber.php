...
class PRSubscriber extends Model
{
    ...
    {
        return $this->hasMany('Acelle\Model\PRSubscriberField');
    }
    ...
    public static function boot()
    {
        ...
        static::creating(function ($item) {
            // Create new uid
            $uid = uniqid();
            while (PRSubscriber::where('uid', '=', $uid)->count() > 0) {
                $uid = uniqid();
            }
            $item->uid = $uid;
        });
    ...
    public function getRules()
    {
        ...
        $rules['EMAIL'] = $rules['EMAIL'].'|unique:subscribers,email,'.$item_id.',id,list_id,'.$this->mailList->id;
    ...
    public function updateFields($params)
    {
        foreach ($params as $tag => $value) {
            ...
            if (is_object($field)) {
                $fv = PRSubscriberField::where('subscriber_id', '=', $this->id)->where('field_id', '=', $field->id)->first();
                if (!is_object($fv)) {
                    $fv = new PRSubscriberField();
    ...
    public static function filter($query, $request)
    {
        ...
        */
        $query = $query->leftJoin('p_r_lists', 'p_r_subscribers.p_r_list_id', '=', 'p_r_lists.id');
        ...
        if (isset($request)) {
            ...
            if (!empty($filters)) {
                if (!empty($filters['status'])) {
                    $query = $query->where('p_r_subscribers.status', '=', $filters['status']);
                }
    ...
    public static function search($request, $customer = null)
    {
        ...
        $query = self::select('p_r_subscribers.*', 'email_verifications.result as verify_result');
        $query = $query->where('p_r_lists.customer_id', '=', $customer->id);
    ...
    {
        $fv = PRSubscriberField::where('p_r_subscriber_id', '=', $this->id)->where('p_r_field_id', '=', $field->id)->first();
        if (is_object($fv)) {
            return $fv->value;
        } else {
            return '';
    ...
    {
        $fv = PRSubscriberField::leftJoin('p_r_fields', 'p_r_fields.id', '=', 'p_r_subscriber_fields.field_id')
            ->where('p_r_subscriber_id', '=', $this->id)->where('p_r_fields.tag', '=', $tag)->first();
        if (is_object($fv)) {
            return $fv->value;
        } else {
            return '';
    ...
    public function setField($field, $value)
    {
        ...
        if (!is_object($fv)) {
            $fv = new PRSubscriberField();
            $fv->p_r_field_id = $field->id;
            $fv->p_r_subscriber_id = $this->id;
        }
    ...
    public static function import($user, $list)
    {
        ...
        foreach ($lines as $key => $line) {
            ...
            // authorize
            if ($user->cannot('create', new \Acelle\Model\PRSubscriber(['p_r__list_id' => $list->id]))) {
                ...
                if ($valid) {
                    ...
                    foreach ($parts as $key => $value) {
                        ...
                        if(isset($headers[$key])) {
                            ...
                            if (is_object($lf)) {
                                //// save fields
                                $lfv = new \Acelle\Model\PRSubscriberField(array(
                                                'p_r_field_id' => $lf->id,
                                                'p_r_subscriber_id' => $subscriber->id,
                                                'value' => $value,
                                            ));
                                $lfv->save();
                            }
