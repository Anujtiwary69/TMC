...
class PRCampaign extends Model
{
    // Campaign status
    ...
    {
        if ($this->isAuto()) {
            return $this->autoEvent()->automation->belongsTo('Acelle\Model\PRList', 'default_mail_list_id');
        } else {
            return $this->belongsTo('Acelle\Model\PRList', 'default_mail_list_id');
        }
    }
    ...
    {
        return $this->hasMany('Acelle\Model\PRCampaignsListsSegment');
    }
    ...
    public function getListsSegments()
    {
        ...
        if($lists_segments->isEmpty()) {
            $lists_segment = new PRCampaignsListsSegment();
    ...
    public function getPendingSubscribers()
    {
        ...
        */
        ...
        $subscribers = $this->subscribers()
                ->whereRaw(sprintf(table('p_r_subscribers').'.id NOT IN (SELECT p_r_subscriber_id FROM %s WHERE p_r_campaign_id = %s)', table('tracking_logs'), $this->id))
                ->whereRaw(DB::getTablePrefix()."p_r_subscribers.status = '".Subscriber::STATUS_SUBSCRIBED."'")
                ->select('p_r_subscribers.email')
                ->addSelect('p_r_subscribers.uid')
                ->addSelect('p_r_subscribers.id')
                ->get();
    ...
    public static function boot()
    {
        ...
        static::creating(function ($item) {
            // Create new uid
            $uid = uniqid();
            while (PRCampaign::where('uid', '=', $uid)->count() > 0) {
                $uid = uniqid();
            }
            $item->uid = $uid;

            // Update custom order
            PRCampaign::getAll()->increment('custom_order', 1);
            $item->custom_order = 0;
        });
    ...
    public function subscribers($params=[])
    {
        ...
        if($this->isAuto()) {
            ...
            $query = PRSubscriber::select("p_r_subscribers.*");
            // Email verification join
            $query = $query->leftJoin('email_verifications', 'email_verifications.p_r_subscriber_id', '=', 'p_r_subscribers.id');
            ...
            foreach($this->listsSegments as $lists_segment) {
                if(!empty($lists_segment->segment_id)) {
                    ...
                    $conditions[] = '(' . table('p_r_subscribers') . '.p_r_list_id = ' . $lists_segment->mail_list_id . ')';
                }
            }
        ...
        if (isset($params["keyword"]) && !empty(trim($params["keyword"]))) {
            foreach (explode(' ', trim($params["keyword"])) as $keyword) {
                $query = $query->leftJoin('p_r_subscriber_fields', 'p_r_subscribers.id', '=', 'p_r_subscriber_fields.p_r_subscriber_id');
                $query = $query->where(function ($q) use ($keyword) {
                    $q->orwhere('p_r_subscribers.email', 'like', '%'.$keyword.'%')
                        ->orWhere('p_r_subscriber_fields.value', 'like', '%'.$keyword.'%');
                });
            }
        }

        return $query->distinct('p_r_subscribers.email');
    }
    ...
    public function clickedEmailsCount()
    {
        ...
        return $query->distinct('p_r_subscriber_id')->count('p_r_subscriber_id');
    }
    ...
    {
        return $this->feedbackLogs()->distinct('p_r_subscriber_id')->count('p_r_subscriber_id');
    }
    ...
    {
        return $this->bounceLogs()->distinct('p_r_subscriber_id')->count('p_r_subscriber_id');
    }
    ...
    {
        return $this->campaign_bounce_logs()->where('bounce_type', '=', 'hard')->distinct('p_r_subscriber_id')->count('p_r_subscriber_id');
    }
    ...
    {
        return $this->campaign_bounce_logs()->where('bounce_type', '=', 'soft')->distinct('p_r_subscriber_id')->count('p_r_subscriber_id');
    }
    ...
    {
        return $this->unsubscribeLogs()->distinct('p_r_subscriber_id')->count('subscriber_id');
    }
    ...
    {
        return \Acelle\Web\PRSubscriber::selectRaw(\DB::getTablePrefix().'list_subscriber.*, COUNT('.\DB::getTablePrefix().'campaign_track_unsubscribe.id) AS openCount')
    ...
    {
        $records = PRSubscriber::select(\DB::raw(\DB::getTablePrefix().'subscribers.*, count(*) as `aggregate`'))
        ...
        if(isset($params['lists_segments'])) {
            foreach ($params['lists_segments'] as $key => $param) {
                ...
                if(!empty($param['mail_list_uid'])) {
                    $mail_list = PRList::findByUid($param['mail_list_uid']);
                ...
                if(!empty($param['segment_uids'])) {
                    foreach ($param['segment_uids'] as $segment_uid) {
                        $segment = PRSegment::findByUid($segment_uid);

                        $lists_segment = new PRCampaignsListsSegment();
                        $lists_segment->mail_campaign_id = $this->id;
                        if(is_object($mail_list)) {
                            $lists_segment->mail_list_id = $mail_list->id;
                        }
                        $lists_segment->mail_segment_id = $segment->id;
    ...
    public function saveRecipients($params=[])
    {
        ...
        foreach($lists_segments_groups as $lists_segments_group) {
            if(!empty($lists_segments_group['segment_uids'])) {
                foreach($lists_segments_group['segment_uids'] as $segment_uid) {
                    $segment = PRSegment::findByUid($segment_uid);
                    $data[] = [
                        'p_r_campaign_id' => $this->id,
                        'p_r_list_id' => $lists_segments_group['list']->id,
                        'p_r_segment_id' => $segment->id,
                    ];
                }
            } else {
                $data[] = [
        ...
        // Insert Data
        PRCampaignsListsSegment::insert($data);

        // Save campaign with default list id
        $campaign = PRCampaign::find($this->id);
    ...
    {
        return $this->subscribers()->count('p_r_subscribers.email');
    }
